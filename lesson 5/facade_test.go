...
func (p *CurrentWeatherData) GetByGeoCoordinates(lat, lon float32) (weather *Weather, err error) {
	return p.doRequest(
		fmt.Sprintf("http://api.openweathermap.org/data/2.5/weather?lat=%f&lon=%f&APPID=%s", lat, lon, p.APIkey))
}

func (p *CurrentWeatherData) GetByCityAndCountryCode(city, countryCode string) (weather *Weather, err error) {
	return p.doRequest(
		fmt.Sprintf("http://api.openweathermap.org/data/2.5/weather?q=%s,%s&APPID=%s", city, countryCode, p.APIkey))
}

func (p *CurrentWeatherData) doRequest(uri string) (weather *Weather, err error) {
	client := &http.Client{}
	...
	weather, err = p.responseParser(resp.Body)
	resp.Body.Close()
	return
}
...
func TestOpenWeatherMap_GetByCityAndCountryCode(t *testing.T) {
	...
	fmt.Printf("Temperature in Almaty is %f celsius\n",
